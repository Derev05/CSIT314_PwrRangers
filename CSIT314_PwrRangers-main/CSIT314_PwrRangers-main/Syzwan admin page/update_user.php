...
if (isset($_POST['userId'], $_POST['username'], $_POST['email'], $_POST['dob'], $_POST['role'])) {
    // Get user data from POST
    $userId = $_POST['userId'];
    $username = $_POST['username'];
    $password = $_POST['password']; // Make sure the password is hashed
    $email = $_POST['email'];
    $dob = $_POST['dob'];
    $role = $_POST['role'];
    ...
    $query = "UPDATE users SET username=?, password=?, email=?, dob=?, role=?, is_suspended=? WHERE id=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('sssssii', $username, $password, $email, $dob, $role, $isSuspended, $userId);
