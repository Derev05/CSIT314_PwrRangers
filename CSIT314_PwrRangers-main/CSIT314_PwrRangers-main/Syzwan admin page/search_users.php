...
if (!empty($query)) {
    ...
    $stmt = $conn->prepare("SELECT id, username, password, email, dob, role, created_at FROM users WHERE username LIKE ?");
    $searchQuery = "%" . $query . "%";  // Prepare for LIKE query
    $stmt->bind_param("s", $searchQuery);
} else {
    // If no query, return all users
    $stmt = $conn->prepare("SELECT id, username, password, email, dob, role, created_at FROM users");
}
