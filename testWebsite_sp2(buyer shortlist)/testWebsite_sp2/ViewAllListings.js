...
class ViewAllListings {
	...
	loadAllListings(query = '') {
		fetch('ViewAllListingsController.php?query=' + query)
  ...
  setupEventListeners() {
		...
		$('document').ready(function(){
			$('#navbar').on('input', '#searchCarListing', function() {
			const query = $(this).val();
			});
		});
	  
	  document.querySelector('#navbar').addEventListener('input', () => {
		  if(event.target.id = 'searchCarListing') {
			  const query = event.target.value;
			  this.loadAllListings(query);
		  }
      });
  }
}
